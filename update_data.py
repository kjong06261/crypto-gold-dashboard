...
AI_TOOLS_30 = [
    {"id":"perplexity-pro","name":"Perplexity Pro","cat":"Research","str":"Real-time news & citations"},
    {"id":"claude-3-5-sonnet","name":"Claude 3.5 Sonnet","cat":"Analysis","str":"Logical reasoning & risk"},
    {"id":"deepseek-v3","name":"DeepSeek V3","cat":"Code/Data","str":"Unmatched cost-efficiency"},
    {"id":"gpt-4o","name":"GPT-4o","cat":"Multi-modal","str":"Creative reports & vision"},
    {"id":"gemini-pro","name":"Gemini Pro","cat":"Verification","str":"Google ecosystem sync"}
] + [{"id":f"tool-{i}","name":f"FinTech Tool {i}","cat":"Investment AI","str":f"Feature {i}"} for i in range(6,31)]
...
NAV_HTML = f"""<div class="nav"><strong style="color:var(--accent)">{SITE_NAME}</strong><a href="/index.html">Home</a><a href="/finance.html">Finance</a><a href="/ai_tools.html">Investor AI</a></div>"""

def wrap_page(title, body):
    return f"""<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8">{ADSENSE_HEAD}<meta name="viewport" content="width=device-width,initial-scale=1.0"><title>{title} | {SITE_NAME}</title><style>{BASE_CSS}</style></head><body><div class="container">{NAV_HTML}{body}<footer>© 2025 {SITE_NAME}</footer></div></body></html>"""
